...
plot_casts <- function(tbbl){
    ggplot(tbbl, aes(year, value, group=name, colour=name))+
    annotate(geom = "rect",
             xmin = c(2016, 2021),
...
#load dataframes-------------------
fcast_tbbl <- read_rds(here("processed","fcast_tbbl.RDS"))
fcast_long <- read_rds(here("processed","fcast_long.RDS"))
...
*   **employment** is from the LFS pre 2021, and from the LMO forecast for 2021 onwards.
*   **2016:2019 ratio** is $\frac{\sum \mbox{New Registrations}}{\sum \mbox{employment}}$ for the years 2016, 2017, 2018, 2019.
*   **2021:2023 ratio** is $\frac{\sum \mbox{New Registrations}}{\sum \mbox{employment}}$ for the years 2021, 2022, 2023.
*   **both period ratio** is $\frac{\sum \mbox{New Registrations}}{\sum \mbox{employment}}$ for the years 2016, 2017, 2018, 2019, 2021, 2022, 2023.
*   **based on 2016:2019 ratio** is employment times 2016:2019 ratio.
*   **based on 2021:2023 ratio** is employment times 2021:2023 ratio.
*   **based on both periods** is employment times both period ratio.
